...
Fig. \ref{fig:PCGIterations} shows the number of PCG iterations needed to reach a stopping tolerance of 0.000001 as a function of the number of elements. Because the PCG method converges as \(N\), it was found that, for any value of \(N\), the PCG solver converges in \(N\) iterations. This behavior is even observed (with small deviations) with higher-order elements. For instance, with 100 quadratic elements, 211 PCG iterations are required, while there are 201 nodes. For cubic elements, a similar scaling is observed, but for linear elements, it was always observed that \(N\) iterations were required. 

\begin{figure}[H]
  \centering
  \includegraphics[width=11cm]{PCGIterations.jpg}
