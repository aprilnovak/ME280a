...
K_condensed_rows = zeros(num_nodes - length(dirichlet_nodes), num_nodes);
F_condensed = zeros(num_nodes - length(dirichlet_nodes), 1);
...
K_condensed = zeros(num_nodes - length(dirichlet_nodes), num_nodes - length(dirichlet_nodes));
...
for K_column = 1:num_nodes
    if (find(dirichlet_nodes == K_column))
        i = i + 1;
    else
